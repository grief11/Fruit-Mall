...
public class LoginController extends BaseController {
    ...
    public String uIndex(Model model, Item item,HttpServletRequest request){
        ...
        List<ItemCategory> pagers = itemCategoryService.listBySqlReturnEntity(sql1);
        ArrayList<CategoryDto> list = new ArrayList<>();
        if (!CollectionUtils.isEmpty(pagers)){
            for (ItemCategory ic : pagers){
        ...
        List<Item> items = itemService.listBySqlReturnEntity("select * from item where isDelete=0 and zk is not null order by zk desc limit 0,10");
